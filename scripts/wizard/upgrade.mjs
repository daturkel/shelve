...
async function upgradeWorker(rl) {
  ...
  try {
    ...
    if (res.status === 401) {
      ui.info("Health check requires a token this script doesn't have — skipping the version comparison.");
      return;
    }
...
async function upgradeWeb(rl) {
  ...
  if (!wizardConfig.pagesProjectName) return; // never deployed via the wizard — nothing to upgrade
