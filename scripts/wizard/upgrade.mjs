...
async function upgradeWeb(rl) {
  ...
  if (!projectName) {
    ...
    // wasn't deployed via this wizard (e.g. docs/SETUP.md's Option B).
    ...
    const hasManualDeploy = await confirm(
      rl,
      "\nNo web app deployment recorded by this wizard — did you deploy one manually (docs/SETUP.md's Option B)?",
      false,
    );
