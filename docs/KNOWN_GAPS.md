...
- **The wizard can't list all Workers in your account** — `wrangler` has no account-wide "list all Workers" command (unlike D1 databases and Pages projects, which it can list). Reconnecting `npm run wizard:deploy` to an existing Worker after losing local config (`worker/wrangler.toml` is gitignored) means typing its name from memory or the Cloudflare dashboard, or via `--worker-name=<name>`. The wizard does check whether that specific name already has deployments before deploying, and warns before redeploying over it.
